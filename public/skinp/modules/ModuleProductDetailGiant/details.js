function details (moduleid, layout, allProductQuantity, productSkus, productSkusAll,ShowFootNav,showZoomer,ImgBig,ProductName) {
    ...
    $('#module_' + moduleid + ' .enquiry').click(function(){
        $('#enquiryDailog'+moduleid).show(0,function() {
            $('#showcallback'+moduleid).data('enquiryCallBack') && $('#showcallback'+moduleid).data('enquiryCallBack')(ProductName,$('[name="ProductID"]').val(),'/comdata/'+getCookie('InitSiteID')+'/product/'+ImgBig,moduleid)
        });
        ...
        module.find('.specification-box .specification-AttrValName').off('click').on('click', function () {
            ...
            if (module.find('.specification-box').length == module.find('.specification-AttrValName.active').length) {
                ...
                if (productSkus[0] != undefined) {
                    ...
                    module.find('.pro-price .price-text').text(price);
                };
            }
        });
    ...
    if (layout == 102 || layout == 103 || layout == 105) {
        ...
        module.find('.introduceBottom').find('.pro-style-containt').off('click').on('click', function (e,data) {
            ...
            if (len == sumLen) {
                ...
                if (productSkus[0] != undefined) {
                    ...
                    // layout=='102'?module.find('.mobile-price').html('￥'+shoulePrice):module.find('.mobile-price').html('<span>￥</span>'+shoulePrice);
        ...
        oInput.off('blur').on('blur', function () {
            if (module.find('.introduceBottom').find('.pro-style-containt.repertory-active').length == sumLen) {
                ...
                if (Number(buyNums) >= productQuantity) {
                    Number(buyNums) > productQuantity ? alert('库存不够') : "";
        ...
        module.find('.mobile-joinCar').off('click').on('click', function () {
            ...
            if ($(window).width() < 991) {
                ...
                if($("#pro-bigPic .ProVideoDiv").find('video') > 0 ) {
            ...
            oDialog.closest('.dialog-mobile-content').animate({
                ...
                if (isSpecification) {
                    ...
                    if ($.inArray(parseInt(layout), [105]) === -1) {
                        ...
                        module.find('.product-spec').html('产品规格&nbsp;&nbsp;' + sChooseSpecification)
                    }
                }
            });
        }
    ...
    module.find('.gallery-thumbs .swiper-slide').off('mouseenter').on('mouseenter', function () {
        ...
        if(module.find('.specification-AttrValName-wb .specification-AttrValName.active').length > 0 || module.find('.specification-container .have-repertory.repertory-active').length > 0){
            module.find('#pro-bigPic .inSwiper-slide').eq(currentbigpic).css({
    ...
    function windowResize (flag) {
        ...
        if (window.innerWidth < 992 && ShowFootNav == 1) {
            $('#MobileFootNav').removeClass('hide');
    ...
    function toastMore108() {
        if (layout == 108) {
            ...
            module.on('click', '.popover .mob-enquiry', function () {
              $('#enquiryDailog'+moduleid).show(0,function() {
                $('#showcallback'+moduleid).data('enquiryCallBack') && $('#showcallback'+moduleid).data('enquiryCallBack')(ProductName,$('[name="ProductID"]').val(),'/comdata/'+getCookie('InitSiteID')+'/product/'+ImgBig)
              });
            })
        }
    }
